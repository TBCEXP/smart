...
check "phase3_verify" "bash '$SCRIPT_DIR/phase3_verify.sh' '$BASE' >/dev/null"
check "phase4_verify" "bash '$SCRIPT_DIR/phase4_verify.sh' '$BASE' >/dev/null"
check "phase5_verify" "bash '$SCRIPT_DIR/phase5_verify.sh' '$BASE' >/dev/null"
check "readiness 含 business" "echo '$READY' | grep -q phase5_inspection_seeded"
...
echo ""
echo "建议操作:"
echo "  1. GitHub 上 Review PR #3 → Merge to main"
echo "  2. 合并后 VPS: cd /opt/smart-crm && bash scripts/upgrade.sh"
